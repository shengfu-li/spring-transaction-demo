...
public class MultiThreadTransactionTest {
    ...
    @Autowired
    private Booking1Service booking1ServiceOne;

    @Autowired
    private Booking1Service booking1ServiceTwo;
     ...
     * propagation_required表示被调用的方法必须在一个具有事务的上下文中运行，如果调用者有事务在进行，那么被调用的方法将在该事务中运行，否则的话重新开启一个事务。
     * <p>
     * Q: booking1ServiceOne.addRequired() 没有抛出异常，booking1ServiceTwo.addRequiredException()抛出了异常，运行结果是什么？
     * A: “张三”都会插入成功，“李四”会插入失败。因为外围run()方法没有开启事务，所以booking1ServiceOne.addRequired()与booking1ServiceTwo.addRequired() 会分别开启一个事务。
     * booking1ServiceTwo.addRequiredException()抛出异常不会影响booking1ServiceOne.addRequired()的独立事务
     *
     * @throws InterruptedException
     */
    @Test
     ...
     * booking1ServiceTwo.addRequiredException()抛出的异常会导致booking1ServiceOne.addRequired()、以及insert_propagation_required()方法的回滚
     *
     * @throws InterruptedException
     */
...
//    }
     ...
     *
     * Q: booking2ServiceOne.addRequiresNew() ，booking2ServiceTwo.addRequiresNewException()都没有抛出异常，外围方法抛出了RuntimeException，运行结果是什么？
