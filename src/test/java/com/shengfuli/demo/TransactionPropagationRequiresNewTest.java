...
import com.shengfuli.demo.service.BookingRequiresNewService;
...
public class TransactionPropagationRequiresNewTest {
    ...
    @Autowired
    BookingRequiresNewService bookingRequiresNewServiceOne;

    @Autowired
    BookingRequiresNewService bookingRequiresNewServiceTwo;
     ...
     *
     * Q: bookingRequiresNewServiceOne.addRequiresNew() 与 bookingRequiresNewServiceTwo.addRequiresNew()没有抛出异常，外围方法抛出了异常，运行结果是什么？
     * A: “张三”、“李四”两条记录都会插入成功。因为外围方法没有开启事务，所以booking2ServiceOne.addRequiresNew()与booking2ServiceTwo.addRequiresNew() 会分别开启一个事务。
     *    外围方法抛出异常不会影响booking2ServiceOne.addRequiresNew()、bookingRequiresNewServiceTwo.addRequiresNew()两者的独立事务
     */
    @Test
    public void  propagation_requires_new(){
        bookingRequiresNewServiceOne.addRequiresNew("张三");
        bookingRequiresNewServiceTwo.addRequiresNew("李四");
     ...
     * propagation_requires_new表示被调用的方法会在自己的事务中运行，Spring会为该被调用的方法新建一个事务。如果存在一个现有的事务，那么在被调用方法执行期间，该事务会被挂起。
     *
     * Q: bookingRequiresNewServiceTwo.addRequiresNewException()抛出异常了，运行结果是什么？
    ...
    public void  propagation_requires_new_exception(){
        bookingRequiresNewServiceOne.addRequiresNew("张三");
        bookingRequiresNewServiceTwo.addRequiresNewException("李四");
    }
    @Test
     ...
     * propagation_requires_new表示被调用的方法会在自己的事务中运行，Spring会为该被调用的方法新建一个事务。如果存在一个现有的事务，那么在被调用方法执行期间，该事务会被挂起。
     *
     *Q: bookingRequiresNewServiceOne.addRequiresNew() 与 bookingRequiresNewServiceTwo.addRequiresNew()都没有抛出异常，外围方法抛出了异常，运行结果是什么？
     *A: “张三”、“李四”两条记录都会插入成功。虽然外围方法开启了事务，但是booking2ServiceOne.addRequiresNew()与booking2ServiceTwo.addRequiresNew() 都在自己独立的事务里。
     *   外围方法抛出异常不会导致booking2ServiceOne.addRequiresNew()、bookingRequiresNewServiceTwo.addRequiresNew()两个方法回滚
     *
     */
    @Test
